...
interface Props {
  visible: boolean;
  isDark: boolean;
  /** The plan currently being edited. Name and entries are seeded from this each time the modal opens. */
  plan: Plan | null;
  /** Current wall-clock time in ms */
  now: number;
  onCancel: () => void;
  /** Called when the user presses Save, with the plan's new name and full entry list. */
  onSave: (name: string, entries: PlannedEntry[]) => void;
  /** Called when the user taps "Delete Plan" — the modal closes immediately, then the parent runs its own confirmation. */
  onDeletePlan: () => void;
}
...
export function EditPlanModal({ visible, isDark, plan, now, onCancel, onSave, onDeletePlan }: Props) {
  ...
  // Re-seed from the live plan every time the modal opens
  useEffect(() => {
  ...
  function handleSave() {
    onSave(name.trim() || plan?.name || 'Plan', entries);
  }

  function handleDeletePlanPress() {
    // Close this modal first, then let the parent's own confirmation Alert run —
    // mirrors how the chip bar's Rename dialog handles "Delete Plan" today.
    onCancel();
    onDeletePlan();
  }
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleClose}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.sheet, { backgroundColor: bgColor }]}>
          ...
          <Text style={[styles.title, { color: textColor }]}>Edit Plan</Text>
          ...
          <Text style={[styles.sectionLabel, { color: subColor }]}>
            {entries.length > 0 ? `Substances (${entries.length})` : 'Substances'}
          </Text>
          ...
          >
            {entries.length === 0 ? (
              <Text style={[styles.emptyText, { color: subColor }]}>No substances in this plan yet.</Text>
            ) : (
              entries.map(e => (
                <StagedEntryRow
                  key={e.id}
          ...
          <TouchableOpacity onPress={handleDeletePlanPress} style={styles.deletePlanBtn}>
...
const styles = StyleSheet.create({
  ...
  footer: { flexDirection: 'row', borderTopWidth: StyleSheet.hairlineWidth, paddingTop: 14, gap: 10 },
  ...
  deletePlanBtn: { paddingTop: 10, paddingBottom: 30, alignItems: 'center' },
  deletePlanText: { fontSize: 14, fontWeight: '600', color: '#FF3B30' },
});
